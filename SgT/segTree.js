class SegmentTree {
    ...
    updateWrapper(tree, index, l, r, u,v,val, BT)
    {
        if (v < l || r < u) {
            return;
        }
        ...
        const left = this.left(index);
        const right = this.right(index);
        this.updateWrapper(tree, left, l, mid, u , v , val, BT);
        this.updateWrapper(tree, right, mid + 1, r, u, v, val, BT);
        this.tree[index] = Math.min(this.tree[left], this.tree[right]);
    }  
    update(u,v,val,BT,n)
    {
        this.updateWrapper(tree, 0, 0, this.nums.length - 1, u,v,val,BT);
        return this.tree;
    }
